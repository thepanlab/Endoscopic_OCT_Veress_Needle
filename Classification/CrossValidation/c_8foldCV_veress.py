''' 
Veress Needle Classification script 
Justin Reynolds, 8/17/21
CNN to classify images of fat, muscle, abdominal space, and intestine.
Data source: Dr. Tang Lab (OU Biomedical Engineering)
'''
import sys
...
if __name__ == '__main__':
    ...
    path2data = '/gpfs/alpine/bif121/proj-shared/veress/classification/data/preproc_v20210817/'
    fin_images =    'c_images_1D_20210817.npy'
    fin_labels =    'c_labels_20210817.npy'
    fin_vid =       'c_vid_20210817.npy'
    fin_filenames = 'c_filenames_20210817.npy'
    # get args
    try:
    ...
    # get X and y test sets
    test_bool = vid == testvid
    X_test = images_1D[test_bool]
    y_test = labels[test_bool]
    # get X and y train sets
    train_bool = vid != testvid
    X_train = images_1D[train_bool]
    y_train = labels[train_bool]
    ...
    y_eval = model.evaluate(X_test, y_test) # evaluate with X_test
    
    t1_start=perf_counter()
    y_preds = model.predict(X_test) # providing predictions on validation set
    t1_stop=perf_counter()
    interp_time=t1_stop-t1_start

    # display results
    print("\n*** S%d %s results ***"%(testvid, arch))
    ...
    print("interp_time:       "+str(interp_time))
    # save results
    # export model (.h5)
    #model.save(export_path+"S"+str(testvid)+"_V"+str(valvid)+"_"+str(arch)+".h5")
    model.save("%sc_f8_S%d_%s_model.h5"%(export_path, testvid, str(arch))
    #with open(export_path+"c_f8_S"+str(testvid)+"_V"+str(valvid)+"_"+str(arch)+".npy", 'wb') as f:
    with open("%sc_f8_S%d_%s_traintimes.npy"%(export_path, testvid, arch), "wb") as f:
        np.save(f, np.array(train_time))
    with open("%sc_f8_S%d_%s_interptimes.npy"%(export_path, testvid, arch), "wb") as f:
        np.save(f, np.array(interp_time))
    with open("%sc_f8_S%d_%s_y_preds.npy"%(export_path, testvid, arch), "wb") as f:
        np.save(f, np.array(y_preds))
    with open("%sc_f8_S%d_%s_y_eval.npy"%(export_path, testvid, arch), "wb") as f:
        np.save(f, np.array(y_eval))
    with open("%sc_f8_S%d_%s_y_test.npy"%(export_path, testvid, arch), "wb") as f:
        np.save(f, np.array(y_test))
    with open("%sc_f8_S%d_%s_trainhist.npy"%(export_path, testvid, arch), "wb") as f:
            np.save(f, np.array(history.history))

    # export training history (.pickle)
    outfile=open("%sf8_S%d_%s_trainhist.pickle"%(export_path, testvid, arch), "wb")
